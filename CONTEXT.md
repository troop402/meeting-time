...
- **In-App PWA Sheet Presentation:** In-app links to the guide (in the Management View footer and the Import dialog) specify `target="_blank"`. In iOS and Android PWAs, this triggers an in-app browser sheet with a top-corner "Done" / close button, allowing users to consult the guide without leaving or reloading the active meeting screen.
